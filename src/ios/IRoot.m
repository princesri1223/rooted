...
- (bool) jailbroken {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/Applications/Frida.app"])
    {
        return YES;
    }
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/Applications/iHide.app"])
    {
        return YES;
    }
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/Applications/Liberty.app"])
    {
        return YES;
    }
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/Applications/HideJB.app"])
    {
        return YES;
    }
    ...
    NSArray *jailBreakHidePath = [NSArray arrayWithObjects:
      ...
      @"/var/log/apt",
      @"/var/log/syslog",
      @"/var/tmp/cydia.log", nil];
    
    for (NSString *str in jailBreakHidePath) {
        if ((f = fopen([str UTF8String], "r"))) {
            fclose(f);
            return YES;
        }
        fclose(f);
